 ...
 * Universal navigation and settings functionality for all pages
 * Includes modal management, settings storage, and API testing
 */
...
function updateApiStatuses() {
    ...
    if (digikeyStatus) {
        const hasDigikey = storage.getDigikeyApiKey();
        const status = (hasDigikey.clientId && hasDigikey.clientSecret) ? 'Active' : 'Inactive';
        digikeyStatus.textContent = status;
        digikeyStatus.className = `status status-${status.toLowerCase()}`;
    }
    
    if (mouserStatus) {
        const hasMouser = storage.getMouserApiKey();
        const status = hasMouser ? 'Active' : 'Inactive';
        mouserStatus.textContent = status;
        mouserStatus.className = `status status-${status.toLowerCase()}`;
    }
}
