...
**TASK ID: ${result.id}**
Name: ${result.name || result.description || 'unnamed'}
Status: ${result.status || 'pending'}

TO UPDATE THIS TASK, USE THIS EXACT ID:
<update_task id="${result.id}" status="ongoing|complete" notes="progress notes" />`;

            case 'task_update': {
                if (!result.success) {
                    const tasks = storageManager.getTasks();
                    const availableIds = tasks.map(t => `${t.id} (${(t.description || t.name || '').slice(0, 40)})`).slice(0, 5);
                    const suggestion = availableIds.length > 0
                        ? `\n\nAVAILABLE TASK IDs:\n${availableIds.map(id => `  • ${id}`).join('\n')}\n\nCheck the "Current Task IDs" section in your context.`
                        : '\n\nNo tasks exist. Create one first with <create_task>.';

                    return `? TASK UPDATE FAILED: Task "${result.id}" NOT FOUND${suggestion}`;
                }

                const statusLabel = result.updates?.status ? this.formatTaskStatus(result.updates.status) : null;
                const parts = [`? TASK UPDATED: ${result.id}`];
                if (statusLabel) parts.push(`Status ? ${statusLabel}`);
                if (result.updates?.notes) parts.push(`Notes: ${result.updates.notes.slice(0, 80)}`);
                return parts.join('\n');
            }
            case 'goal_create':
...
## CRITICAL: Task ID Usage Rules (READ CAREFULLY)
1. **NEVER invent task IDs**. Task IDs are generated by the system in format: task-XXXX (e.g., task-E642, task-XK6X).
2. **DO NOT use** patterns like task_1, task_2, task-1, task-2. These will FAIL.
3. When <create_task> succeeds, you receive the ID in the tool result: "TASK ID: task-E642". **COPY THIS EXACT ID**.
4. Every iteration shows "CURRENT TASK IDs" at the top. **USE THOSE EXACT IDs** for <update_task>.
5. If you get "Task not found", check "CURRENT TASK IDs" section - you used a wrong ID.

## Task ID Verification Pattern (MANDATORY BEFORE EVERY UPDATE)
<reasoning_text>
I need to update the task about "Analyze Option C".
Looking at CURRENT TASK IDs section:
  1. ID: task-E642, Status: [pending], Desc: Analyze Option C
  2. ID: task-XK6X, Status: [pending], Desc: Analyze Option D

The correct ID for "Analyze Option C" is: task-E642
I will now update it.
</reasoning_text>

<update_task id="task-E642" status="ongoing" notes="Starting analysis of Option C..." />

## Common Mistakes to AVOID
❌ WRONG: <update_task id="task_1" .../>  (underscore format - doesn't exist)
❌ WRONG: <update_task id="task-1" .../>  (sequential number - doesn't exist)
❌ WRONG: Guessing IDs without checking CURRENT TASK IDs section
✓ CORRECT: Check "CURRENT TASK IDs" → Copy exact ID → Use in update_task
...
## Lab API Reference (JavaScript Execution Context)
...
const individuals = Lab.value('data-XYZ');
const filtered = individuals.filter(p => p.age > 18);

// Store result
const token = Lab.store(filtered, {
    label: 'Adults only',
    tags: ['filtered', 'individuals']
});

// Later: retrieve and continue processing
const adults = Lab.value(token);
...
const fnMeta = Lab.value('data-FN123');
const actualFn = eval(fnMeta.source); // Reconstruct function
const result = actualFn(arg1, arg2);

**Data Type Examples:**
// Strings longer than 500 chars are auto-vaulted
const longText = "very long content...";
const token = Lab.store(longText, {label: 'Long report'});

// Arrays and objects
const users = [{name: 'Alice', age: 30}, {name: 'Bob', age: 25}];
const usersToken = Lab.store(users, {label: 'User list', tags: ['users']});

// Later retrieval
const loadedUsers = Lab.value(usersToken);
const names = loadedUsers.map(u => u.name); // ['Alice', 'Bob']

// Maps and Sets
const myMap = new Map([['key1', 'value1'], ['key2', 'value2']]);
const mapToken = Lab.store(myMap, {label: 'Settings map'});
const loadedMap = Lab.value(mapToken); // Restored as Map object

**Error Prevention:**
// ALWAYS validate data after loading
const data = Lab.value('data-XYZ');
if (!data || !Array.isArray(data)) {
    throw new Error('Data is not an array or failed to load');
}
...
Proceed with disciplined reasoning. The current workspace state follows.`;
    ...
    buildIterationPrompt(userMessage, iteration) {
        ...
        // CRITICAL: Task IDs at the very top
        if (tasks.length > 0) {
            lines.push('═══════════════════════════════════════════════════');
            lines.push('CURRENT TASK IDs (USE THESE EXACT IDs FOR UPDATES):');
            lines.push('═══════════════════════════════════════════════════');
            tasks.forEach((t, idx) => {
                const desc = (t.description || t.name || '').slice(0, 50);
                lines.push(`${idx + 1}. ID: ${t.id}`);
                lines.push(`   Status: [${t.status}]`);
                lines.push(`   Desc: ${desc}`);
                if (t.notes) lines.push(`   Notes: ${t.notes.slice(0, 60)}`);
                lines.push('');
            });
            lines.push('⚠️  CRITICAL: Copy the EXACT ID above (e.g., task-E642, NOT task_1)');
            lines.push('═══════════════════════════════════════════════════');
            lines.push('');
        }

        lines.push('## Operating Reminder:');
        lines.push('- Use the XML tools exactly as defined (no JavaScript helpers for memory, goals, or tasks).');
        lines.push('- Wait for the system-provided "Tool Activity" summaries before relying on tool results.');
        lines.push('- **ALWAYS use task IDs from the "CURRENT TASK IDs" section above**');
        lines.push('- When you encounter [[vault:...]] references, inspect them with <vault_read id="vault_id" mode="preview" limit="1200" />');
        lines.push('- Wrap all narrative thoughts inside <reasoning_text>...</reasoning_text>');
        lines.push('');

        lines.push('## Lab API Quick Reference (for execute_js):');
        lines.push('- Lab.value(id) → Get actual JS object (for .filter, .map, etc.)');
        lines.push('- Lab.read(id) → Get string preview (for display only)');
        lines.push('- Lab.store(data, {label}) → Save data, returns [[vault:id]]');
        lines.push('- Lab.list() → See all vault entries');
        lines.push('⚠️  Common mistake: Lab.read() returns strings, not objects. Use Lab.value() for data manipulation.');
        lines.push('');
        ...
        if (tasks.length > 0) {
            lines.push('## Detailed Task Information:');
            tasks.forEach(t => {
                lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
                ...
                lines.push(`Created: ${this.formatRelativeTime(t.createdAt)}`);
                lines.push(`Updated: ${this.formatRelativeTime(t.updatedAt)}`);
                lines.push('');
                lines.push(`TO UPDATE: <update_task id="${t.id}" status="..." notes="..." />`);
            });
            lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
            lines.push('');
        }
